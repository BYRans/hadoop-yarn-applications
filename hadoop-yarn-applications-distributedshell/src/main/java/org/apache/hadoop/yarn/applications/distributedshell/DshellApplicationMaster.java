...
public class DshellApplicationMaster {
    ...
    private static final Log LOG = LogFactory.getLog(DshellApplicationMaster.class);
    ...
    private static final String ExecShellStringPath = DshellClient.SCRIPT_PATH + ".sh";
    ...
    public boolean init(String[] args) throws ParseException, IOException {
        ...
        if (fileExist(log4jPath)) {
            try {
                Log4jPropertyHelper.updateLog4jConfiguration(DshellApplicationMaster.class,
                        log4jPath);
            } catch (Exception e) {
        ...
        if (!fileExist(shellCommandPath)
                && envs.get(DshellDSConstants.DISTRIBUTEDSHELLSCRIPTLOCATION).isEmpty()) {
            throw new IllegalArgumentException(
        ...
        LOG.info(">>>>>>>>>>>>>>>>> Loading container files:");
        for (String key : envs.keySet()) {
            if (key.contains(DshellDSConstants.DISTRIBUTEDJARLOCATION)) {
                ...
                LOG.info("\tLoaded container files:"+scistorJar.getJarPath());
            }

        }

        LOG.info(">>>>>>>>>>>>>>>>> Loading container archives:");
        for (String key : envs.keySet()) {
            if (key.contains(DshellDSConstants.DISTRIBUTEDARCHIVELOCATION)) {
                ...
                LOG.info("\tLoaded container files:"+scistorArchive.getArchivePath());
            }
        }
        // ↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
        ...
        if (envs.containsKey(DshellDSConstants.DISTRIBUTEDSHELLTIMELINEDOMAIN)) {
            domainId = envs.get(DshellDSConstants.DISTRIBUTEDSHELLTIMELINEDOMAIN);
        }
    ...
    public void run() throws YarnException, IOException {
        ...
        LOG.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>> Start launche containers!");
        for (int i = 0; i < numTotalContainersToRequest; ++i) {
            ContainerRequest containerAsk = setupContainerAskForRM();
            amRMClient.addContainerRequest(containerAsk);
        }
        LOG.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>> Launched all containers already!");
    ...
    private class LaunchContainerRunnable implements Runnable {
        ...
        public void run() {
            ...
            LOG.info(">>>>>>>>>>>Loading files.File list size: "+scistorJars.size());
            for (DshellFile perJar : scistorJars) {
                ...
                LOG.info(">>>>>>>>>>>>>>>>put key："+tmp[(tmp.length - 1)]+"-value"+jarRsrc.getResource()+" to local resources.");
                localResources.put(tmp[(tmp.length - 1)], jarRsrc);
            }
            String[] tmp;
            LOG.info(">>>>>>>>>>>Loading archives.archive list size: "+scistorArchives.size());
            for (DshellArchive perArchive : scistorArchives) {
                ...
                LOG.info(">>>>>>>>>>>>>>>>put key："+tmptmp[0]+"-value"+archiveRsrc.getResource()+" to local resources.");
            ...
            LOG.info(">>>>>>>>>>>>>>>>>Container set up finished,ready to start async.");
